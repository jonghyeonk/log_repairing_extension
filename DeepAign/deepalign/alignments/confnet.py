...
class ConfNet:
    ...
    def predict(self, inputs_f, inputs_b):
        out_f = self.net_f.predict(inputs_f[0])
        out_b = self.net_b.predict(inputs_b[0])
    ...
    def fit(self, dataset, batch_size=32, **kwargs):
        ...
        h1 = self.net_f.fit(dataset.features[0], dataset.targets[0], batch_size=batch_size, **kwargs)
        dataset.reverse(True)
        h2 = self.net_b.fit(dataset.features[0], dataset.targets[0], batch_size=batch_size, **kwargs)
        return h1, h2
