...
for(type in c('trd', 'edit')){
  ...
  ####
  setwd(paste0("~/log_repairing_extension/alignment/result_", type))
  align= read.csv(paste(data, "_", rate[i], ".csv", sep='' ),T)
...
write.csv(result, paste0(data, "_align_", type , ".csv"), row.names = F)
}
}
