...
recon_rework = function(repair, actlist, place_rework, longerfirst, k , e2_set, actset, num_place){
  ...
  if(length(which(apply(vote[[1]],2,sum)>1)) < num_place){
    print(paste("Prediction = [rework] at level",  paste(error, collapse = ",")))
...
recon_insert_A = function(repair, actlist, longerfirst, k, error, 
  ...
  if(length(err_insert) <= num_place){
    print(paste("Prediction = [insert] at level", paste(error, collapse = ",")))
...
recon_insert_B = function(repair, actlist, longerfirst, j, k, 
...
recon_skip = function(sliding_check, actlist, repair, variant_frame2,
                      j, k, error, actset, num_place, 
                      replace_change, flag_adjust_moved,
                      inserted_seq, inserted_loc){
  ...
  if(sum(priority1) == 0 & (replace_change>0 | 
    ...
    vote = voting_matrix(repair, actset,  model.nodes, model.edges, base.list)
    if(length(which(apply(vote[[1]],2,sum)>1)) <= num_place){
      print(paste("Prediction = [skip] at level", paste(error, collapse = ",")))
...
recon_moved_replace = function(inserted_seq, inserted_loc, 
  ...
  if(length(check_seq)>0){
    ...
    PBARs[[jj]]$deleted_seq <<- 'REMOVED'
    PBARs[[jj]]$predict_pattern <<- '-'
    
    predict_pattern[j] <<- 'moved'
    predict_pattern <<- predict_pattern[-jj]
    # predict_pattern <<- predict_pattern[-which(predict_pattern=='insert')[1]]
    deleted_seq <<- deleted_seq[-which(is.element(deleted_seq, check_seq[1]))[1]]
    PBARs[[j]]$predict_pattern <<- 'moved'
